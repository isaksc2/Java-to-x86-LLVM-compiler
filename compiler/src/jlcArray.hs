...
main = do
  ...
  -------------------------- for debugging ----------------------------------------
  --program <- readFile "../../test/testsuite/good/core005.jl"

   -------------------------- for debugging array ----------------------------------------
  --program <- readFile "../../test/testsuite/extensions/arrays1/array004.jl"
...
check s = do
  case pProg (myLexer s) of
    Bad err  -> do
      ...
      hPutStrLn stderr err -- syntax error
      exitFailure
    Ok  tree -> do
      case typecheck tree of
        Bad err -> do
          ...
          hPutStrLn stderr err -- type error 
          exitFailure
        Ok tree -> do
          hPutStrLn stderr "OK"
          writeX86 tree
