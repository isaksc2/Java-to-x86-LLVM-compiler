...
--prefixMulOp (Lit Doub) Div   = "fdiv" -- todo dont need?
prefixMulOp (Lit Int) Div    = "sdiv"
--prefixMulOp (Lit Int) Mod    = "srem" -- todo dont use
...
instance ToLLVM Code where
  toLLVM = \case
    ...
    Label l                                -> toLLVM l ++ ":"
    Cmp t v1 v2 | t == Lit Doub -> "cmpsd" ++ (sizeKeyword t v1) ++ " " ++ toLLVM v1 ++ ", " ++ toLLVM v2
   ...
   -- GetElementPointer r' s r i             -> toLLVM r' ++ " = getelementptr " ++ stringType s ++ ", " ++ stringType s ++ "* " ++ toLLVM r ++ ", " ++ toLLVM i
    ...
    Push t v | t == Lit Doub ->     "push __?float32?__("++ toLLVM v  ++")"     -- "push dword " ++ toLLVM v 
...
registerAlloc = do
  ...
  -- real registers
  let iRegs       = [RCX, RDI, RSI, R8, R9, R10, R11, R12, R13, R14, R15]
...
emitBinaryOp t op' e1 e2 b = do
  ...
  case op' of
    ...
    Mo Div -> divide'      r (Lit t) arg2 arg1
    Mo Mod -> modulo'      r (Lit t) arg2 arg1
  ...
  where
    ...
    --modulo
    modulo' :: Register -> LLVMType -> Value -> Value -> Compile ()
    modulo' r t arg2 arg1 = do
      emit $ Mov t arg2 (X86 RAX) -- todo correct order?
      emit $ Mov t (LitInt 0) (X86 RDX)
      emit $ Mov t arg1 (X86 RBX) 
      emit $ CDiv t (X86 RBX)
      emit $ Mov t (X86 RDX) (Reg r)

    -- divide: move denom to rbx, divide and move result from RAX to r.
    divide' :: Register -> LLVMType -> Value -> Value -> Compile ()
    divide' r t arg2 arg1 = do
      emit $ Mov t arg2 (X86 RAX) -- todo correct order?
      emit $ Mov t arg1 (X86 RBX)
      emit $ CDiv t (X86 RBX)
      emit $ Mov t (X86 RAX) (Reg r)
