...
typecheck (PDefs defs) = do
  ...
  annotatedTree <- checkDefs emptyEnv (std++defs)
  return ()
...
updateVar _ id Type_void = Bad $ "can't have void as type for varaible: " ++ (show id)
...
updateVar (sig,(c:cons)) id typ = do
  if (Map.member id (head (c:cons)))
    then
      Bad $ "variable exists already" ++ (show id)
    else
      return (sig, (Map.insert id typ c):cons)

updateFun :: Env -> Id -> ([Type],Type) -> Err Env
updateFun (sig,cons) id (args, result) = do
  if (Map.member id sig)
    then
      Bad $ "function exists already" ++ (show id)
    else
      return (Map.insert id ((args, result)) sig, cons)
...
inferExp env x = case x of
  ...
  EApp id args1 -> do
    ...
    if(length argsT /= length args2)
      then
        Bad $ "incorrect amount of arguments for function " ++ (show id)
      else do
        let pairs = zip argsT args2
        let argsT' = coerceTargets pairs
        if (checkTypes argsT' args2)
          then
            return (ETyped x result)
          else
            Bad $ "arguments have incorrect type for function " ++ (show id)
  ...
  EPost id op -> do
    ...
    if (typ == Type_int || typ == Type_double)
      then
        ...
        Bad $ "type " ++ printTree typ ++ "of variable " ++ show id ++ " incompatible with operator " ++ show op
  ...
  ECmp exp1 op exp2 -> do
    ...
    if((op /= OEq && op /= ONEq ) && typ3 == Type_bool)
      then
        Bad $ "can't compare bools: " ++ printTree exp1 ++ " and " ++ printTree exp2
      else
        return (ETyped (ECmp exp3 op exp4) Type_bool)
  ...
  EAss id exp -> do
    ...
    if (typ == typ')
      then
        ...
        Bad $ "variable " ++ show id ++ " has type " ++ printTree typ ++ ", cant assign expression of type " ++ printTree typ'
...
coerceTargets [] = []
coerceTargets (((ETyped exp typ),target):ts) = coerceTarget exp typ target:coerceTargets ts

coerceTarget :: Exp -> Type -> Type -> (Exp,Type)
coerceTarget e  Type_int Type_double = ((ETyped e Type_double), Type_double)
coerceTarget e t _ =  (e,t)

coerce :: (Exp,Type) -> (Exp,Type) -> Err (Exp,Exp,Type)
coerce (e1, Type_int) (e2, Type_double) = return ((ETyped e1 Type_double), e2 , Type_double)
coerce (e1, Type_double) (e2, Type_int) = return (e1, (ETyped e2 Type_double), Type_double)
coerce (e1, t1) (e2, t2) | t1==t2 = return (e1, e2, t1)
                         | otherwise = Bad $ "failed when casting " ++ printTree e1 ++ " and " ++ printTree e2 ++ ",can't cast " ++ printTree t1 ++" to "++ printTree t2 ++ " or vice versa"
...
inferBin types env exp1 exp2 = do
  ...
  if (elem typ1 types && elem typ2 types)
    then do
      ...
      Bad $ "type of " ++ printTree exp1 ++ " type of " ++ printTree exp2
        ++ " expected types from" ++ (show types)
          ++ " but found " ++ printTree typ1 ++ " and " ++ printTree typ2
...
addSigs env ((DFun result (Id "main") args ss):ds) = do
  if(result /= Type_int)
    then
      Bad "main must have type int"
    else do
      if(args /= [])
        then
          Bad "main must have no arguments"
        else do
